...
const CURATED_IMAGE_LIBRARY = {
    ...
    tshirt: [
        ...
        'https://images.unsplash.com/photo-1490111718993-d98654ce6cf7?auto=format&fit=crop&w=800&q=80'
    ],
    shirt: [
    ...
    sweater: [
        'https://images.unsplash.com/photo-1504593811423-6dd665756598?auto=format&fit=crop&w=800&q=80',
        'https://images.unsplash.com/photo-1500631195312-e3a9a5819f60?auto=format&fit=crop&w=800&q=80'
    ],
    dress: [
        'https://images.unsplash.com/photo-1500631195312-e3a9a5819f60?auto=format&fit=crop&w=800&q=80',
    ...
    eyewear: [
        'https://images.unsplash.com/photo-1522312298940-653d8a0b22c3?auto=format&fit=crop&w=800&q=80',
        'https://images.unsplash.com/photo-1490598000245-075175152d25?auto=format&fit=crop&w=800&q=80'
    ],
    scarf: [
        'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=800&q=80',
        'https://images.unsplash.com/photo-1503341455253-b2e723bb3dbb?auto=format&fit=crop&w=800&q=80'
...
function getProductImage(image, category, subcategory, title) {
    ...
    // Allow absolute paths and absolute URLs
    ...
    // Generate unique, deterministic image based on product title
    if (title) {
        const seedString = `${title}-${category || ''}-${subcategory || ''}`;
        const seed = hashString(seedString);
        const garmentKey = detectGarmentKey(title, subcategory, category);
        const curatedPool = CURATED_IMAGE_LIBRARY[garmentKey] || [];
        if (curatedPool.length) {
            const index = seed % curatedPool.length;
            return curatedPool[index];
        }
        const keywords = generateKeywords(title, category, subcategory);
        return `https://source.unsplash.com/featured/800x800/?${encodeURIComponent(keywords)}&sig=${seed}`;
    }
    // Deterministic fallback when no title or external sources fail
    return getFallbackImage(title, category, subcategory);
}
function getFallbackImage(title, category, subcategory) {
    const seedString = `${title || 'product'}-${category || ''}-${subcategory || ''}`;
    const seed = hashString(seedString);
...
//# sourceMappingURL=_5ed54bed._.js.map