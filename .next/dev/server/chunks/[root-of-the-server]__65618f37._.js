...
"[project]/lib/chatbot-data.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
// Generate tags
const getTags = (product, index, total)=>{
    const tags = [];
...
const transformProduct = (product, index, total)=>{
...
const getProductDataset = ()=>{
    if (cachedDataset) return cachedDataset;
    const filePath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(process.cwd(), 'data', 'products.json');
    try {
    ...
    const enhancedProducts = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$products$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["products"].map((p, i)=>transformProduct(p, i, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$products$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["products"].length));
    cachedDataset = {
    ...
    try {
        ...
        __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].writeFileSync(filePath, JSON.stringify(cachedDataset, null, 2), 'utf8');
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$chatbot$2d$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/chatbot-data.ts [app-route] (ecmascript)");
;
;
...
const findExactMatch = (dataset, query)=>{
    const normalizedQuery = query.toLowerCase().trim();
    // Try exact name match first
    const exactMatch = dataset.products.find((product)=>product.name.toLowerCase() === normalizedQuery);
    if (exactMatch) return exactMatch;
    // Try partial exact match (query is subset of product name)
    const partialExact = dataset.products.find((product)=>{
        const productName = product.name.toLowerCase();
        return productName === normalizedQuery || productName.includes(normalizedQuery) || normalizedQuery.includes(productName);
    });
    if (partialExact) return partialExact;
    return null;
};
// Search products with advanced filtering
const searchProducts = (dataset, query, intent)=>{
    ...
    let results = dataset.products.filter((product)=>{
        ...
        if (productKeywords.length > 0) {
            ...
            const matchesKeyword = productKeywords.some((keyword)=>productName.includes(keyword) || productDesc.includes(keyword) || productSubcat.includes(keyword));
...
const findSimilarProducts = (product, dataset, limit = 4)=>{
    return dataset.products.filter((p)=>p.id !== product.id && (p.category === product.category || p.subcategory === product.subcategory || Math.abs(p.price - product.price) < product.price * 0.3)).sort((a, b)=>{
        const aScore = (a.category === product.category ? 10 : 0) + (a.subcategory === product.subcategory ? 5 : 0) + (Math.abs(a.price - product.price) < product.price * 0.2 ? 3 : 0);
        const bScore = (b.category === product.category ? 10 : 0) + (b.subcategory === product.subcategory ? 5 : 0) + (Math.abs(b.price - product.price) < product.price * 0.2 ? 3 : 0);
        return bScore - aScore;
    }).slice(0, limit);
};
// Generate redirect URL based on intent/query
const getRedirectUrl = (intent, query, category)=>{
...
// Generate response
const generateResponse = (query, results, dataset, intent)=>{
    const category = extractCategory(query);
    // Non-product query
    ...
    if (intent === 'count') {
        if (category) {
            const count = dataset.products.filter((p)=>p.category === category).length;
            return {
    ...
    if (results.length === 0) {
        ...
        if (productKeywords.length > 0) {
            ...
            similar = allProducts.filter((p)=>p.category === category).slice(0, 4);
        }
        if (similar.length > 0) {
            return {
                text: "❌ Sorry, aapke search ke according exact product nahi mila.\n\n✔ Similar options:\n\n",
                products: similar.map((p, i)=>({
                        ...p,
    ...
    if (exactMatch && intent === 'search') {
        const p = exactMatch;
        const discountText = p.discount > 0 ? `₹${p.price.toLocaleString('en-IN')} (${p.discount}% off, was ₹${p.originalPrice.toLocaleString('en-IN')})` : `₹${p.price.toLocaleString('en-IN')}`;
...
async function POST(request) {
    try {
        ...
        const dataset = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$chatbot$2d$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getProductDataset"])();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__65618f37._.js.map